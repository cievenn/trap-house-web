    ...
    <section id="vip" className="relative w-full min-h-screen py-20 md:py-32 lg:py-48 px-6 z-20 overflow-hidden">
      <div className="max-w-7xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-24 text-center flex flex-col items-center">
          ...
          <p className="font-manrope text-white/60 text-sm md:text-base max-w-lg mx-auto uppercase tracking-widest">
            {"L'accès est restreint. Le contenu est exclusif."}
          </p>
        </motion.div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
          {/* CARTE SHOP — role="button" for a11y */}
          <motion.div
            ...
            role="button"
            tabIndex={0}
            aria-label="Accéder au Trap House Shop — Bientôt disponible"
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') e.currentTarget.click(); }}
            className="interactive-element relative h-auto min-h-[400px] sm:min-h-[450px] md:h-[550px] flex flex-col items-center justify-center rounded-[2.5rem] border border-white/5 overflow-hidden group bg-[#020202] shadow-[0_20px_50px_rgba(0,0,0,0.5)] hover:border-white/10 transition-colors duration-500 outline-none focus-visible:ring-2 focus-visible:ring-[#00F2FF] focus-visible:ring-offset-2 focus-visible:ring-offset-black"
          >
            {/* Static noise overlay — replaces expensive SVG feTurbulence filter */}
            ...
            <div className="relative z-20 flex flex-col items-center p-6 md:p-10 text-center w-full">
              ...
              <p className="font-manrope text-white/60 text-sm mb-12 max-w-sm leading-relaxed group-hover:text-white/70 transition-colors">
              ...
              <div className="relative px-6 sm:px-10 py-4 rounded-full bg-white/5 border border-white/10 group-hover:border-[#00F2FF]/80 overflow-hidden transition-all duration-500 cursor-pointer">
                ...
                <p className="relative z-10 text-white/60 group-hover:text-black font-syne text-[10px] sm:text-[11px] tracking-[0.4em] font-black uppercase transition-colors duration-500">Coming Soon</p>
              </div>
            </div>
          ...
          {/* CARTE VIP RÉSERVATIONS — role="button" for a11y */}
          <motion.div
            initial={{ opacity: 0, y: 50, rotateX: 5 }}
            whileInView={{ opacity: 1, y: 0, rotateX: 0 }}
            viewport={{ once: true }}
            ...
            role="button"
            tabIndex={0}
            aria-label="Accéder aux Réservations VIP — Bientôt disponible"
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') e.currentTarget.click(); }}
            className="interactive-element relative h-auto min-h-[400px] sm:min-h-[450px] md:h-[550px] flex flex-col items-center justify-center rounded-[2.5rem] border border-white/5 overflow-hidden group bg-[#020202] shadow-[0_20px_50px_rgba(0,0,0,0.5)] hover:border-white/10 transition-colors duration-500 outline-none focus-visible:ring-2 focus-visible:ring-[#00F2FF] focus-visible:ring-offset-2 focus-visible:ring-offset-black"
          >
            {/* Static noise overlay — replaces expensive SVG feTurbulence filter */}
